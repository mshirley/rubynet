...
connected -- prints info about current nodes
"       
	when "auth?"
		return "authok"
	else
		puts "unknown return code for #{input}"
...
def del_connected(nodename, nodeip, nodeport)
        puts "del_connection function executed"
        $connected << "nodename=#{nodename}:nodeip=#{nodeip}:nodeport=#{nodeport},"
        return
end
